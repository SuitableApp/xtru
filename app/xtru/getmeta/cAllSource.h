 ...
 * @class cAllSource
 * @brief
 * provides the subset data of ALL_SOURCE.
 *
 * @details
 ...
 * @code
    ...
    ...
    ps::app::xtru::getmeta::cAllSource oObj;
    oQueue_.push_back(oObj.oSubmit(*oSvc_, oOwners_));
    ps::lib::vSynchronize(iConcurrency_, oQueue_);
    std::cout << oPrintItems(oObj, std::make_tuple("SCOTT", "MY_PACKAGE", "PACKAGE BODY"), true) ;
   @endcode
 */
class cAllSource
{
private:
    ...
    /// @brief A SQL-select to select ALL_SOURCE
    static const char szAllSource[];
public:
    /**
     * @brief
     */
