 ...
 * @class cAllTriggers
 * @brief
 * provides the subset data of ALL_TRIGGERS
 *
 * @details
...
private:
    ...
    typedef void (cAllTriggers::tAttributes::*tOnRepeatAction)(
        std::ostream&
        , const size_t&
        , cAllTriggers&
    ) const;
    /// @copydoc cAllSequences::tOneTimeAction
    typedef void (cAllTriggers::*tOneTimeAction)(
        std::ostream&
        , const tAttributes&
    ) const;
    ...
    /// @brief A SQL-select to select ALL_TRIGGERS
    static const char szAllTriggers[];
public:
    /**
     * @brief
     */
    cAllTriggers();
    virtual ~cAllTriggers();
    /**
     * @brief
     * @param [in,out] oSvc
