...
/**
 * @class cAllIndColumns
 * @brief
 * provides the subset data of ALL_IND_COLUMNS
 *
 * @details
 ...
 * @code
    ...
    ...
    ps::app::xtru::getmeta::cAllIndColumns oObj(oAllTabColumns, oAllIndExpressions);
    oQueue_.push_back(oObj.oSubmit(*oSvc_, oOwners_));
    ps::lib::vSynchronize(iConcurrency_, oQueue_);
    std::cout << oPrintItems(oObj, std::make_tuple("SCOTT", "TEST18", "TEST18_PK"), true) ;
   @endcode
 */
class cAllIndColumns
{
private:
    ...
    typedef void (cAllIndColumns::tAttributes::*tOnRepeatAction)(
        std::ostream&
        , const size_t&
        , cAllIndColumns&
    ) const;
    /// @copydoc cAllSequences::tOneTimeAction
    typedef void (cAllIndColumns::*tOneTimeAction)(
        std::ostream&
        , const tAttributes&
    ) const;
    ...
    /// @brief A SQL-select to select ALL_IND_COLUMNS
    static const char szAllIndColumns[];
    cAllTabColumns& oAllTabColumns_;
    cAllIndExpressions& oAllIndExpressions_;
public:
   /**
     * @brief
     */
